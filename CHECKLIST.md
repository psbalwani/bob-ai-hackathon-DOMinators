...
- [x] (New, post-freeze) Drug-level performance aggregation ("would this drug get FDA approval") — rolls up every site's risk score + the drug's deviations + CAPA remediation status into one `drug_risk_index`/`readiness_band` verdict per drug, documented in `src/risk_scoring/DESIGN.md`'s "Drug-Level Aggregation" section (`src/risk_scoring/drug_aggregate.py`). Exposed at `GET /dashboard/drug-performance` (Track D gateway, real CAPA data) and `GET /risk-score/drug-summary` (Track B standalone, no CAPA data — factor dropped/redistributed rather than assumed zero-risk). New frontend tab/page: `src/frontend/src/pages/DrugPerformance.tsx`, added to the sidebar in `Layout.tsx`. Verified live against the real dataset (`GET /dashboard/drug-performance?protocol_id=TRIAL-2026-ONC-04` returns real aggregated numbers incl. real CAPA counts) and via `tests/test_drug_aggregate.py` (5/5 pass: clearly-approval-ready drug, clearly-high-risk drug, missing-CAPA-data redistribution, zero-sites edge case, rationale-never-hallucinates-a-number). Full suite still 181/181 passing after this change.
...
- [x] Visual/UX polish pass across all 4 dashboard pages — Inter/JetBrains Mono webfonts, card elevation tokens (`shadow-card`), gradient sidebar logo mark + "Monitor" nav grouping, iconified stat tiles, risk-band-colored score ring on the site drill-down, a shared `Button` component replacing ad-hoc button classes, consistent hover/active states throughout. Same routes/data flow/contracts, no backend changes. Verified live against real backend data (18 sites, port 8000 gateway) via Playwright screenshots at desktop, mobile, and dark-mode — `src/frontend/src/index.css`, `components/{Layout,Card,StatTile,Button,icons}.tsx`, all 4 `pages/*.tsx`
...
- [x] Admin "All Drugs" overview page (`src/frontend/src/pages/AdminOverview.tsx`) — lets a multi-drug owner (e.g. `owner_portfolio`) see every owned drug's dashboard summary + FDA-readiness verdict in one table, without switching or re-logging-in per drug; clicking a row switches `currentProtocolId` and drills into that drug's Trial Overview. Purely additive on the frontend (reuses the existing per-protocol `/dashboard/summary` and `/dashboard/drug-performance` endpoints in parallel, no backend changes); nav item (`Layout.tsx`, new `IconLayers`) only shown when the logged-in owner has >1 protocol. Verified live in a real browser (Playwright driver): the nav item appears only for `owner_portfolio` (not `owner_onc04`), the table renders real per-drug stats and readiness bands for all 10 drugs, and clicking a row correctly navigates into that drug's dashboard — zero console errors
