...
- `llm_hook.py` — the one hook to watsonx.ai, used only for that ambiguous boundary. If `WATSONX_API_KEY` isn't set (or the call fails for any reason), it returns `None` and `severity.py` falls back to a fixed conservative default (`Minor`). The whole test suite is designed to pass on this fallback path with zero external calls.
...
```bash
pytest tests/test_rules.py tests/test_detector_recall.py tests/test_api.py -v
```
